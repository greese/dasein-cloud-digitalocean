 ...
 * @since 2012.09
 */
public class DOProvider implements Comparable<DOProvider> {
    ...
    static public final DOProvider AWS          = new DOProvider(AWS_NAME);
    static public final DOProvider ENSTRATUS    = new DOProvider(ENSTRATUS_NAME);
    static public final DOProvider OPENSTACK    = new DOProvider(OPENSTACK_NAME);
    static public final DOProvider EUCALYPTUS   = new DOProvider(EUCALYPTUS_NAME);
    static public final DOProvider OTHER        = new DOProvider(OTHER_NAME);
    static public final DOProvider STORAGE      = new DOProvider(STORAGE_NAME);

    static private Set<DOProvider> providers;

    static public @Nonnull DOProvider valueOf(@Nonnull String providerName) {
        if( providerName.equalsIgnoreCase("Amazon") ) {
            return AWS;
        }
        // cannot statically initialize the list of providers
        if( providers == null ) {
            TreeSet<DOProvider> tmp = new TreeSet<DOProvider>();
    ...
    static public @Nonnull Set<DOProvider> values() {
        return providers;
    }
    ...
    public boolean equals(@Nullable Object ob) {
        return ob != null && (ob == this || getClass().getName().equals(ob.getClass().getName()) && providerName.equals(((DOProvider) ob).providerName));
    }
